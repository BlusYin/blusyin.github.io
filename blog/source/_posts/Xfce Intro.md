...
`xfce4-clipman`，剪贴板动作可以通过正则表达式匹配剪贴版内容，根据匹配结果弹出可执行的指令供选择。这相比于KDE的*Web Shortcut*功能扩展性更强，但是不支持手动输入内容，也不能集成到搜索功能中，而且还是被动触发。

以打开Collions网页查询单词为例。打开Applications->Settings->Clipboard Manager Settings，打开Actions选项卡，点击“+”添加一个action，弹出窗口中，首先给Action命名为“Word Definition”，然后输入要匹配的正则表达式“[a-zA-Z- ]+”，并勾选*Activate only on manual copy*，否则该动作不会生效。接着是Command部分，每个action可以有多个Command，每个Command都有一个名称和对应的指令。这里添加一个Command，命名为“Collins Dictionary”， 指令为`exo-open https://www.collinsdictionary.com/dictionary/english/\0`，其中“\0”会在指令执行时被所匹配到的内容替代。
